package servlet;

import java.io.IOException;
import java.util.ArrayList;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import DAO.*;

/**
 * Servlet implementation class manageItems
 */
@WebServlet("/manageItems")
public class manageItems extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public manageItems() {
        super();
        // TODO Auto-generated constructor stub
    }

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		Itemes item = new Itemes();
		int itemid = Integer.parseInt(request.getParameter("action"));
		if (item.isBlocked(itemid)) {
			item.changeItemStateByItemId(itemid, 1);
			request.getRequestDispatcher("sellerManageItem.jsp").forward(request, response);
		} else {
			item.changeItemStateByItemId(itemid, 0);
			request.getRequestDispatcher("sellerManageItem.jsp").forward(request, response);
		}
		
		
	}

}
